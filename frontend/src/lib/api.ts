...
// Generate Realistic Land-Bound Spatial Incidents
function generateLocalIncidents(centerLat: number, centerLng: number, count: number = 300): Incident[] {
  ...
  const isLagos = Math.abs(centerLat - 6.52) < 0.15 && Math.abs(centerLng - 3.37) < 0.15;
  ...
  for (let i = 0; i < count; i++) {
    const clusterOffsetLat = (Math.floor(i / 30) - 4) * 0.015;
    const clusterOffsetLng = ((i % 10) - 5) * 0.015;

    let lat = centerLat + clusterOffsetLat + (Math.random() - 0.5) * 0.008;
    let lng = centerLng + clusterOffsetLng + (Math.random() - 0.5) * 0.008;

    // Land-bound safety filter for coastal cities (Accra Gulf of Guinea is south of Lat 5.545)
    if (isAccra && lat < 5.545) {
      lat = 5.545 + Math.random() * 0.04;
    }
    if (isLagos && lat < 6.43) {
      lat = 6.43 + Math.random() * 0.05;
    }
...
export async function getBlackspots(): Promise<Blackspot[]> {
  ...
  const offsets: { dLat: number; dLng: number; count: number; sev: number; risk: number; factors: Record<string, number> }[] = [
    { dLat: 0.012, dLng: 0.015, count: 42, sev: 2.1, risk: 88, factors: { 'Speeding': 18, 'Wet Surface': 14, 'Unlit Junction': 10 } },
    { dLat: -0.010, dLng: 0.018, count: 35, sev: 1.8, risk: 76, factors: { 'Heavy Traffic': 15, 'Daylight': 12, 'Overtaking': 8 } },
    { dLat: 0.018, dLng: -0.012, count: 28, sev: 2.4, risk: 92, factors: { 'Darkness': 12, 'High Speed': 10, 'Sharp Curve': 6 } },
    { dLat: -0.015, dLng: -0.015, count: 22, sev: 1.5, risk: 64, factors: { 'Rain': 9, 'T-Junction': 8, 'Pedestrian Crossing': 5 } },
  ];
